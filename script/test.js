...
document.getElementById('add-to-cart').addEventListener('click', function () {
    ...
    let currentCartQty = parseInt(cartQtyElement.innerText);
    const cartCount = document.getElementById('selected-qty')
    let currentSelectedCount = parseInt(cartCount.innerText);

    if (currentCartQty > 0) {
        checkoutBtn.classList.remove('hidden');
        currentSelectedCount = currentCartQty + currentSelectedCount;
        cartCount.innerHTML = currentSelectedCount;

        // const selectedColor = querySelectorAll('.border-purple-700');
...
const modaldark = document.getElementById('modal-dark-bg')

darkbg.addEventListener('click', function (event) {
    if (event.target === modaldark) {
        modalBox.classList.add('hidden')
    }
})
